...
import { getUserByIdApi, saveUserApi, deleteUserByIdApi } from '@/api/user'
...
const { tableRegister, tableState, tableMethods } = useTable({
  ...
  fetchDelApi: async () => {
    const res = await deleteUserByIdApi({ ids: unref(ids) })
    return !!res
  }
})
...
const crudSchemas = reactive<CrudSchema[]>([
  ...
  },
  {
    ...
    table: {
      ...
      slots: {
        default: (data: any) => {
          const row = data.row as UserInfo
          return (
            <>
...
const delData = async (row?: UserInfo) => {
  const elTableExpose = await getElTableExpose()
  ids.value = row ? [row.Id] : elTableExpose?.getSelectionRows().map((v: UserInfo) => v.Id) || []
