...
import { ChipPackagingList } from '@/api/chipPackaging/types'

// 使用设计系统
const { getPrefixCls } = useDesign()
const prefixCls = getPrefixCls('chip-packaging-table')

// 定义 props
const props = defineProps({
  tableData: {
    type: Array as PropType<ChipPackagingList[]>,
    default: () => []
  },
  tableType: {
...
// 定义 emits
const emit = defineEmits(['selection-change', 'wip-qty-click', 'receipt-qty-click'])
...
// 选择变化事件处理
const handleSelectionChange = (selection: ChipPackagingList[]) => {
  emit('selection-change', selection)
}

// 处理wipQty点击事件
const handleWipQtyClick = (docNo: string) => {
  emit('wip-qty-click', docNo)
}
        ...
        >
          <template #default="{ row }">
            ...
            <span
              ...
              @click="handleWipQtyClick(row.docNo)"
            >
            ...
            <!-- receiptQty字段可点击 -->
            <span
              v-else-if="column.prop === 'receiptQty' && row[column.prop] > 0"
            ...
            <!-- 日期字段格式化 -->
            <span
              v-else-if="['packagingDate', 'firstArrivalDate'].includes(column.prop)"
...
@prefix-cls: ~'@{adminNamespace}-chip-packaging-table';
</style>
