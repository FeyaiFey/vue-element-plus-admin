...
import ChipPackagingReceiptTable from './components/ChipPackagingReceiptTable.vue'
import { ChipPackagingReceiptQuery } from '@/api/chipPackaging/types'
import {
  getChipPackagingReceiptListApi,
  exportChipPackagingReceiptListApi
} from '@/api/chipPackaging'

// 使用设计系统
const { getPrefixCls } = useDesign()
const prefixCls = getPrefixCls('chip-packaging-receipt-list')

// 定义组件名称（用于keep-alive缓存）
defineOptions({
  name: 'ChipPackagingReceipt'
})
...
const searchParams = ref<ChipPackagingReceiptQuery>({
  pageIndex: 1,
  pageSize: 20,
  itemCode: undefined,
...
const advancedSearchParams = ref<{
  ...
  arrivalDateStart?: string | undefined
  arrivalDateEnd?: string | undefined
}>({
  purchaseNo: undefined,
  packageType: undefined,
  lotCode: undefined,
...
const { tableState, tableMethods } = useTable({
  fetchDataApi: async () => {
    const res = await getChipPackagingReceiptListApi(searchParams.value)
    return {
      list: res.data.list || [],
...
const handleResetAdvanced = () => {
  ...
  // 清空主搜索参数中的高级搜索部分
  searchParams.value.purchaseNo = undefined
  searchParams.value.packageType = undefined
  searchParams.value.lotCode = undefined
  searchParams.value.supplier = undefined
  searchParams.value.arrivalDateStart = undefined
  searchParams.value.arrivalDateEnd = undefined
...
const calculateSummary = () => {
  ...
  const priceQtySum = rows.reduce((sum, row) => sum + (Number(row.priceQty) || 0), 0)
  const priceSum = rows.reduce((sum, row) => sum + (Number(row.price) || 0), 0)
  const amountSum = rows.reduce((sum, row) => sum + (Number(row.amount) || 0), 0)
  const priceAvg = rows.reduce((sum, row) => sum + (Number(row.price) || 0), 0) / count
...
const handleDownload = async () => {
  try {
    ...
    // 调用导出API，传入当前搜索条件
    const response = await exportChipPackagingReceiptListApi({
      ...searchParams.value
    })
    ...
    let filename = `chipPackagingReceipts_${new Date().toLocaleDateString()}.xlsx`
      ...
      <div class="text-sm text-gray-600 dark:text-gray-300">
        ...
        <div v-else class="flex flex-col gap-1">
          <div class="flex items-center gap-4">
            ...
            <span>数量: {{ formatNumber(summaryData.priceQtySum, 0) }}</span>
            <span>金额: {{ formatNumber(summaryData.priceSum) }}</span>
          </div>
          <div class="flex items-center gap-4 text-xs">
            <span>总价: ¥{{ formatNumber(summaryData.amountSum) }}</span>
      ...
      <div class="p-4">
        <ElForm :model="advancedSearchParams" label-width="80px" label-position="left">
          <ElFormItem label="订单号">
            <ElInput
              v-model="advancedSearchParams.purchaseNo"
              placeholder="请输入订单号"
              clearable
            />
          </ElFormItem>
          ...
          <ElFormItem label="打印批号">
            <ElInput
              v-model="advancedSearchParams.lotCode"
              placeholder="请输入打印批号"
              clearable
            />
          </ElFormItem>
          ...
          <ElFormItem label="到货开始">
            <ElDatePicker
              v-model="advancedSearchParams.arrivalDateStart"
              type="date"
              placeholder="请选择开始日期"
          ...
          <ElFormItem label="到货结束">
            <ElDatePicker
              v-model="advancedSearchParams.arrivalDateEnd"
              type="date"
              placeholder="请选择结束日期"
              value-format="YYYY-MM-DD"
              clearable
...
@prefix-cls: ~'@{adminNamespace}-chip-packaging-receipt-list';
</style>
