...
import { ChipPackagingReceipt } from '@/api/chipPackaging/types'

// 使用设计系统
const { getPrefixCls } = useDesign()
const prefixCls = getPrefixCls('chip-packaging-receipt-table')

// 定义 props
const props = defineProps({
  tableData: {
    type: Array as PropType<ChipPackagingReceipt[]>,
    default: () => []
  },
  tableType: {
...
// 选择变化事件处理
const handleSelectionChange = (selection: ChipPackagingReceipt[]) => {
...
// 计算选中行的合计
const calculateSelectionSummary = (selection: ChipPackagingReceipt[]) => {
  const priceQtySum = selection.reduce((sum, item) => sum + (Number(item.priceQty) || 0), 0)

  return {
    count: selection.length,
    priceQtySum: formatNumber(priceQtySum)
  }
}
</script>
        ...
        <div class="selection-summary-row">
          <div class="summary-content">
            ...
            <span class="summary-item"
              >数量合计: <span class="summary-value">{{ summaryData.priceQtySum }}</span></span
            >
          </div>
        </div>
      </template>
      ...
      <template v-for="column in currentColumns" :key="column.prop">
        ...
        >
          <template #default="{ row }">
            <!-- 数量字段格式化 -->
            <span
              v-if="['priceQty'].includes(column.prop)"
            ...
            <!-- 日期字段格式化 -->
            <span
              v-else-if="column.prop === 'arrivalDate'"
...
@prefix-cls: ~'@{adminNamespace}-chip-packaging-receipt-table';
</style>
