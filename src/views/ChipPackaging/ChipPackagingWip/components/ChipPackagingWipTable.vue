...
import { ChipPackagingWip } from '@/api/chipPackaging/types'

// 使用设计系统
const { getPrefixCls } = useDesign()
const prefixCls = getPrefixCls('chip-packaging-wip-table')

// 定义 props
const props = defineProps({
  tableData: {
    type: Array as PropType<ChipPackagingWip[]>,
    default: () => []
  },
  tableType: {
...
// 获取状态标签类型
const getStatusType = (currentProcess: string): 'info' | 'warning' | 'primary' | 'success' => {
  if (!currentProcess) return 'info'
...
// 选择变化事件处理
const handleSelectionChange = (selection: ChipPackagingWip[]) => {
      ...
      <template v-for="column in currentColumns" :key="column.prop">
        ...
        >
          <template #default="{ row }">
            <!-- 数量字段格式化 -->
            <span
              v-if="['onlineTotal', 'warehouseInventory'].includes(column.prop)"
            ...
            <!-- 当前工站字段格式化 -->
            <ElTag
              v-else-if="column.prop === 'currentProcess' && row[column.prop]"
...
@prefix-cls: ~'@{adminNamespace}-chip-packaging-wip-table';
</style>
