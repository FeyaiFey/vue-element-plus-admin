...
import CpReceiptTable from './components/CpReceiptTable.vue'
import { CpReceiptQuery } from '@/api/cp/type'
import { getCpReceiptListApi, exportCpReceiptListApi } from '@/api/cp'

// 使用设计系统
const { getPrefixCls } = useDesign()
const prefixCls = getPrefixCls('cp-receipt-list')
...
const searchParams = ref<CpReceiptQuery>({
  pageIndex: 1,
  pageSize: 20,
...
const { tableState, tableMethods } = useTable({
  fetchDataApi: async () => {
    const res = await getCpReceiptListApi(searchParams.value)
    return {
      list: res.data.list || [],
...
const handleResetAdvanced = () => {
  ...
  searchParams.value.progressName = undefined
  searchParams.value.programName = undefined
...
const handleDownload = async () => {
  try {
    ...
    // 调用导出API，传入当前搜索条件
    const response = await exportCpReceiptListApi({
      ...searchParams.value
    })
    ...
    let filename = `cpReceipts_${new Date().toLocaleDateString()}.xlsx`
...
@prefix-cls: ~'@{adminNamespace}-cp-receipt-list';
</style>
