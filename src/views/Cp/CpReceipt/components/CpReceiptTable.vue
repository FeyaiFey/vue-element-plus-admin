...
import { CpReceipt } from '@/api/cp/type'

// 使用设计系统
const { getPrefixCls } = useDesign()
const prefixCls = getPrefixCls('cp-receipt-table')

// 定义 props
const props = defineProps({
  tableData: {
    type: Array as PropType<CpReceipt[]>,
    default: () => []
  },
  tableType: {
...
// 选择变化事件处理
const handleSelectionChange = (selection: CpReceipt[]) => {
...
// 计算选中行的合计
const calculateSelectionSummary = (selection: CpReceipt[]) => {
  const priceQtySum = selection.reduce((sum, item) => sum + (Number(item.receiptQty) || 0), 0)
      ...
      <template v-for="column in currentColumns" :key="column.prop">
        ...
        >
          <template #default="{ row }">
            <!-- 数量字段格式化 -->
            <span
              v-if="['receiptQty'].includes(column.prop)"
...
@prefix-cls: ~'@{adminNamespace}-cp-receipt-table';
</style>
