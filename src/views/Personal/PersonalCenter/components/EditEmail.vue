...
import { updateEmailPasswordApi, getUserEmailInfoApi } from '@/api/user'
...
const save = async () => {
  try {
    ...
    // 调用修改密码的接口
    const res = await updateEmailPasswordApi({
...
const getEmailInfo = async () => {
  try {
    const res = await getUserEmailInfoApi()
    if (res.code === 200) {
      // 设置表单数据
      await setValues({
      ...
      ElMessage.error(res.message || '获取邮箱信息失败')
    }
  } catch (error) {
