...
import { getGlobalReportListApi, exportGlobalReportListApi } from '@/api/report'
import { getStockReportListApi } from '@/api/report'
import { getChipPackagingWipListApi } from '@/api/chipPackaging'
import { getCpTestListApi } from '@/api/cp'
import { getWaferPurchaseWipListApi } from '@/api/waferPurchase'
import StockTable from '@/views/Report/Stock/components/StockTable.vue'
import ChipPackagingWipTable from '@/views/ChipPackaging/ChipPackagingWip/components/ChipPackagingWipTable.vue'
import WaferPurchaseWipTable from '@/views/WaferPurchase/WaferPurchaseWip/components/WaferPurchaseWipTable.vue'
import CpListTable from '@/views/Cp/CpList/components/CpListTable.vue'
import type { GlobalReport } from '@/api/report/types'
    ...
    if (columnConfig) {
      ...
      if (columnConfig.type === 'stock') {
        ...
        if (currentColumn.value.param2_name) {
          ...
          const res = await getStockReportListApi({
        ...
        const res = await getChipPackagingWipListApi({
          [`${paramName1}`]: paramValue1,
          isTr: undefined
        })
        dialogData.value = res.data.list
        ...
        const res = await getWaferPurchaseWipListApi({
          [`${paramName1}`]: paramValue1,
          isFinished: '0'
        })
        dialogData.value = res.data.list
        ...
        const res = await getCpTestListApi({
          [`${paramName1}`]: paramValue1,
          status: '0'
        })
        dialogData.value = res.data.list
...
const getReportData = async () => {
  try {
    loading.value = true
    const res = await getGlobalReportListApi()
...
const handleExport = async () => {
  try {
    ...
    const res = (await exportGlobalReportListApi()) as unknown as AxiosResponse
    ...
    let filename = `globalReport_${new Date().toLocaleDateString()}.xlsx`
