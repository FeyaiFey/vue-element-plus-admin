...
export const getButterflyChartOption = (
  ...
  type: 'detail' | 'summary',
  sortField: SortField = 'FORECAST_QTY',
  ...
  const { categories, forecast, actual } =
    type === 'detail'
      ? processDetailData(data as SaleTargetDetail[], sortField, sortDirection)
      : processSummaryData(data as SaleTargetSummary[], sortField, sortDirection)
...
const processDetailData = (
  ...
  // 使用所有数据，不再限制条数
  return {
    categories: sortedData.map((item) => item.ITEM_NAME),
    forecast: sortedData.map((item) => item.FORECAST_QTY || 0),
    actual: sortedData.map((item) => item.PRICE_QTY || 0)
  }
}

// 处理汇总数据
const processSummaryData = (
  data: SaleTargetSummary[],
  sortField: SortField = 'FORECAST_QTY',
  sortDirection: SortDirection = 'asc'
) => {
  // 根据字段和方向排序
  ...
  // 使用所有数据，不再限制条数
  return {
    categories: sortedData.map((item) => `${item.EMPLOYEE_NAME}`),
    forecast: sortedData.map((item) => item.FORECAST_QTY || 0),
    actual: sortedData.map((item) => item.PRICE_QTY || 0)
  }
}
